...
namespace LQFarm.EditorTools
{
    ...
    public static class MutationTest
    {
        ...
        /// <summary>A legendary must still finish inside one overnight gap, or a good roll becomes
        /// a punishment for sleeping.</summary>
        static void GrowCapHolds(List<string> fails)
        {
            var s = new PlayerState(); s.NewGame(); s.lv = 1;
            int top = Art.Elements.Length - 1;
            ...
            foreach (var seed in GameData.Seeds)
                foreach (Weather w in System.Enum.GetValues(typeof(Weather)))
                {
                    ...
                    float legend = s.GrowTimeIn(seed, w, top);
                    Check(fails, legend >= plain, $"{seed.id}/{w}: đột biến lại nhanh hơn cây thường");
                    Check(fails, legend - plain <= Art.MaxMutationGrowAddSeconds + 1f,
                          $"{seed.id}/{w}: cộng thêm {legend - plain:0}s, trần là {Art.MaxMutationGrowAddSeconds:0}s");
                    Check(fails, legend <= 3600f, $"{seed.id}/{w}: {legend:0}s vượt quá một giờ");
                }
        }
